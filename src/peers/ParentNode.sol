...
import {YieldPeer, Client, IRouterClient, CCIPOperations} from "./BaseYieldNode.sol";
...
contract ParentPeer is YieldPeer {
    ...
    function withdraw(uint256 shareAmount) external {
        ...
        uint256 userShares = i_share.balanceOf(msg.sender);
        ...
        uint256 usdcToWithdraw = _calculateWithdrawAmount(totalValue, totalShares, shareAmount);

        // Update total shares
        s_totalShares -= shareAmount;
        ...
        address activeStrategyAdapter = _getActiveStrategyAdapter();
        if (usdcToWithdraw > 0) {
            _withdrawFromStrategy(activeStrategyAdapter, usdcToWithdraw);
            // Transfer USDC to user
            _transferUsdcTo(msg.sender, usdcToWithdraw);
        }

        emit WithdrawCompleted(msg.sender, usdcToWithdraw);
    }
